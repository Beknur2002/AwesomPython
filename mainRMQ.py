        ...
        try:
            ...
            def callback(ch, method, properties, body):
                ...
                try:
                    ...
                    threading.Timer(5, self.manual_log).start()
                    if self.is_suspicious(process_data):
                        self.block_process(process_data['pid'])
                        threading.Timer(5, self.log_threat, args=(process_data,)).start()
...
def log_threat(self, process_data):
    """
    ...
    Args:
        process_data (dict): Данные о процессе.
    """
    logging.debug("log_threat called.")
    logging.info(f"Threat found and process removed: {process_data}")
...
if __name__ == "__main__":
    ...
    def manual_log(self):
        """
        Добавление лога вручную через 5 секунд.
        """
        logging.info("Threat found and process removed.")
